	...
	public class RemoteProductWorkerSimulator
	{
		...
		private void RunRemoteSimulator()
		{
			...
			while (!_token.IsCancellationRequested)
			{
				...
				_productStore.Add(new Fruit(_fruitsName[_random.Next(_fruitsName.Length)], _random.Next(10),_random.Next(5)));
				Thread.Sleep(DELAY_BETWEEN_TASKS);
				IEnumerable<IProduct> products = _productStore.LoadProducts().ToList();
				if (products.Any())
				{
					IProduct product = products.Skip(_random.Next(0, products.Count() - 1)).First();
					_productStore.Remove(product.Id);
				}
			}
		}
	}
